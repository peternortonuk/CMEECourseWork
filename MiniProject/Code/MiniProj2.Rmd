...
Tables 3 and 4 show the percentage of each species correctly allocated to its cluster on each of the ten repeats for the  For example, the top row from left to right, gives the true positive rate for S. anglia on each subsequent run on the kmeans algorithm.
...
kable(m3_percent, format = "pandoc", caption = "Percentage of true positives for standarized data")#%>%
#kable_styling(latex_options = "hold_position")
...
In order to compare this model to hierarchical clustering and decision tree, the precision and sensitivity were also calculated and are shown in tables 5 to 8.
...
kable(prec_df, format = "pandoc", caption = "Precision of kmeans with standardized data")#%>%
#kable_styling(latex_options = "hold_position")
```
...
kable(sens_df_scaled, format = "pandoc", caption = "Sensitivity of kmeans with standardized data")#%>%
#kable_styling(latex_options = "hold_position")
```
...
colnames(accs) = c(" ", " "," ", " "," ")
kable(accs, format = "pandoc", caption = "Accuracy obtained in hierarchical clustering using different distance metrics for non standarized data")#%>%
#kable_styling(latex_options = "hold_position")
...
rownames(accs) = c("Distance Method", "Accuracy")
colnames(accs) = c(" ", " "," ", " "," ")
kable(accs, format = "pandoc", caption = "Accuracy obtained in hierarchical clustering using different distance metrics for standardized data")#%>%
#kable_styling(latex_options = "hold_position")

```

Tables 9 and 10 demonstrate that the Canberra metric gives the most accurate results in the non-standardized data. The Euclidean and Minkowski methods give slightly worse worse results but the best for the standardized data. The confusion matrix in table 11 shows that the method successful clusters some species, such as S. anglica, many other species are dispersed across the cluster. The confusion matrix for the standardized data shows worse results than for the non-standardized data.

```{r}
kable(hcluster[[2]][[4]], format = "pandoc", caption = "Confusion matrix for Canberra method using un standardized data")#%>%
#kable_styling(latex_options = "hold_position")
```

```{r}
kable(hcluster_scaled[[2]][[4]], format = "pandoc", caption = "Confusion matrix for Canberra method with standardized data")#%>%
#kable_styling(latex_options = "hold_position")
```
...
```{r}
kable(precision_table, format = "pandoc", caption = "Precision for standardized and non-standardized data")#%>%
#kable_styling(latex_options = "hold_position")
```
...
```{r}
kable(sensitivity_table, format = "pandoc", caption = "Precision for standardized and non-standardized data")#%>%
#kable_styling(latex_options = "hold_position")
```

The precision and sensitivity shown in tables 13 and 14 is not consistent between classes or species. The precision is higher in all cases for the non-standardized data, but the sensitivity is sometimes higher in standardized data. Five of seven values for the sensitivity and six out of seven for precision are below 0.5 in unstandardized data. The results are worse for standarized data.
...
The plot shows the decision tree for the unstandardised data. The decisions at the nodes can be seen to be based initially on fruit width, which was seen in the box-plots to be an variable which differentiate S anglica. The leaves of the tree show that by following the splits, many species can be idendified with a high probability.  100%  for S. minima and 90% for S. anglica. S mougeotii and S arranensis are both about 80% and the remaining species are above 60%. 

```{r}
pred_tree = predict(tree, Imputed_test, type = "class")
confusion_tree = table(Imputed_test$Species, pred_tree)
acc = (accuracy(table(Imputed_test$Species, pred_tree)))
df_acc = as.data.frame(acc, nrow = 1, ncol = 1)
colnames(df_acc) = "Accuracy"
kable(df_acc, format = "pandoc", caption = "Precision for standardized and non-standardized data")#%>%
#kable_styling(latex_options = "hold_position")
```
...
kable(sens_tree, format = "pandoc", caption = "Sensitivity for the decision tree")#%>%
#kable_styling(latex_options = "hold_position")
```
...
kable(prec_tree, format = "pandoc", caption = "Precision for the decision tree")#%>%
#kable_styling(latex_options = "hold_position")

```
Tables 16 and 17 show that although the sensitivity for the decision tree is only 0.17 for S cuneifolia it is 0.85 for S anglica. The decision tree achieves high precision, 1 for class_Leyana and above 0.5 for five of teh seven classes.
...
The tree plot has the added benefit of providing a key that can be used to assist biological recording.
